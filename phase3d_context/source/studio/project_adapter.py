            ...
            try:
                ...
                for sh in veo_data.get("shots", []):
                    ...
                    if cur_id == shot_id:
                        ...
                        # Gate B: canonical Shot-level identity. Prefer the shot record's
                        # own image_prompt when present (non-empty); fall back to the
                        # legacy scene-level image_prompts.json prompt otherwise.
                        # Never writes back / duplicates data here — loader is read-only.
                        duration_val = sh.get("duration")
                        image_prompt_val = sh.get("image_prompt")
                        if image_prompt_val is None or (isinstance(image_prompt_val, str) and not image_prompt_val.strip()):
                            image_prompt_val = ""
                            try:
