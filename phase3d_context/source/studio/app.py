...
class ShotPromptUpdate(BaseModel):
    """Shot-level prompt mutation — Gate B: each Shot has independent image_prompt identity.

    Gate C/D: Visual/Motion Blueprint inputs (visual_objective, shot_purpose,
    category, subject_ids, environment_id, prop_ids, constraints, continuity
    anchors, motion fields) are distinct from provider-facing prompts
    (image_prompt, veo_prompt). Changing a committed Blueprint input marks the
    dependent prompt OUTDATED (no auto-regeneration); changing the prompt text
    never rewrites the Blueprint.
    """
    image_prompt: Optional[str] = None
    veo_prompt: Optional[str] = None
    negative_prompt: Optional[str] = None
    # Blueprint inputs (optional — when present they update semantic plan only)
    ...
    lighting_atmosphere: Optional[str] = None
    continuity_anchor: Optional[str] = None
    # Gate H: explicit override for locked shots — lock means "do not silently
    # overwrite". Locked shots return 409 unless override_lock is True.
    override_lock: Optional[bool] = False

    model_config = {"extra": "ignore"}


# Gate C/D: Blueprint input fields whose change marks dependent prompt OUTDATED.
_BLUEPRINT_FIELDS = frozenset({
    "visual_objective", "shot_purpose", "category",
    "subject_ids", "environment_id", "prop_ids", "constraints",
    "camera_motion", "subject_action", "environmental_action",
    "lighting_atmosphere", "continuity_anchor",
})
_PROMPT_FIELDS = frozenset({"image_prompt", "veo_prompt", "negative_prompt"})


@app.patch("/api/projects/{dir_name}/visual/shots/{shot_id}")
async def patch_project_visual_shot_prompt(dir_name: str, shot_id: str, req: ShotPromptUpdate):
    """
    Mutate shot-level prompts independently.
    Gate B compliance: image_prompt is written to the shot record in veo_prompts.json,
    NOT to the scene-level image_prompts.json.  Sibling shots are never affected.
    Gate C/D: Blueprint vs prompt are separate; Blueprint change marks prompt OUTDATED.
    Gate H: locked shots return 409 LOCK_CONFLICT unless override_lock=True.
    Unknown/legacy fields in veo_prompts.json are preserved (only requested keys written).
    """
    from studio.project_adapter import project_adapter
    try:
        project_dir = project_adapter.resolve_project_dir(dir_name)
        veo_path = project_dir / "veo_prompts.json"
        if not veo_path.is_file():
            raise HTTPException(status_code=404, detail=f"veo_prompts.json not found for {dir_name}")

        # Gate H: lock check — do not silently overwrite locked shots.
        try:
            from studio.project_bootstrap import get_project_state_store
            from studio.locking import LockManager
            store = get_project_state_store(dir_name)
            if LockManager(store).is_locked(shot_id) and not (req.override_lock or False):
                raise HTTPException(status_code=409, detail={
                    "code": "LOCK_CONFLICT",
                    "message": f"Shot '{shot_id}' is locked. Unlock or pass override_lock=true.",
                    "shot_id": shot_id,
                })
        except HTTPException:
            raise
        except Exception:
            pass  # lock store unavailable → treat as unlocked (legacy behavior)

        veo_data = json.loads(veo_path.read_text(encoding="utf-8"))
        shots = veo_data.get("shots", [])
        found = False
        updates = req.model_dump(exclude_unset=True)
        updates.pop("override_lock", None)
        blueprint_changed = any(k in _BLUEPRINT_FIELDS for k in updates)
        prompt_explicit = any(k in _PROMPT_FIELDS for k in updates)
        for sh in shots:
            if str(sh.get("shot_id") or "") == shot_id:
                # Write only requested fields; never touch sibling shots.
                # Unknown/legacy fields on every record are preserved.
                for field, val in updates.items():
                    sh[field] = val
                # Gate C/D/H: Blueprint change without explicit prompt rewrite
                # marks the dependent prompt OUTDATED (content stays unchanged,
                # explicit user action required — no auto-regeneration).
                if blueprint_changed and not prompt_explicit:
                    sh["outdated"] = True
                found = True
                break
        ...
        veo_path.write_text(json.dumps(veo_data, ensure_ascii=False, indent=2), encoding="utf-8")
        logger.info(f"Patched shot {shot_id} in {dir_name}: fields={list(updates.keys())}")
        return {"shot_id": shot_id, "updated_fields": list(updates.keys()),
                "blueprint_changed": blueprint_changed, "status": "ok"}
    except HTTPException:
        raise
    except Exception as e:
        logger.exception(f"Failed to patch shot {shot_id} for {dir_name}: {e}")
        raise HTTPException(status_code=500, detail=str(e))


@app.get("/api/projects/{dir_name}/visual/route/{shot_id}")
async def get_visual_shot_route(dir_name: str, shot_id: str):
    """
    Gate E: Deterministic Visual Router.
    Returns the production route for a shot based on visual_prompts.json + veo_prompts.json data.
    No LLM calls — pure rules-based routing from shot category and visual type.
    """
    from studio.project_adapter import project_adapter
    ...
    try:
        shot = project_adapter.load_shot_detail(dir_name, shot_id)
        project_dir = project_adapter.resolve_project_dir(dir_name)
        # Load visual_prompts scene entry for the parent scene
        vp_entry = None
        vp_path = project_dir / "visual_prompts.json"
        if vp_path.is_file():
            try:
                vp_data = json.loads(vp_path.read_text(encoding="utf-8"))
                parent_id = shot.parent_scene_id
                vp_entry = next(
                    (e for e in vp_data.get("entries", []) if e.get("sceneId") == parent_id),
                    None
                )
            except Exception:
                pass
        route = VisualRouter.route(shot, vp_entry)
        return route
    except (FileNotFoundError, KeyError) as fe:
        raise HTTPException(status_code=404, detail=str(fe))
    except Exception as e:
        logger.exception(f"Failed to route shot {shot_id} for {dir_name}: {e}")
        raise HTTPException(status_code=500, detail=str(e))


@app.get("/api/projects/{dir_name}/visual/handoff/{shot_id}")
async def get_visual_shot_handoff(dir_name: str, shot_id: str):
    """Gate G: manual Flow/Veo handoff bundle with actual references.

    Surfaces Shot ID, Image Prompt, Negative Prompt, bound Character /
    Environment / Prop references (stable entity IDs resolved against
    visual_bible.json), style snapshot, Motion Blueprint
    (camera/subject/environment motion, timing, constraints) and the Veo
    Motion Prompt plus approved/start-frame context. Manual — never uploads
    or spends anything automatically.
    """
    from studio.project_adapter import project_adapter
    from studio.visual_router import VisualRouter
    try:
        ...
        def _char_ref(cid: str) -> dict:
            _pool = list(vb.get("characters") or []) + list(vb.get("subjects") or [])
            for pool in _pool:
                if pool.get("characterId") == cid or pool.get("subjectId") == cid or pool.get("id") == cid:
                    return {"entity_id": cid, "name": pool.get("name"),
                            "species": pool.get("species"),
                            "visual_anchors": pool.get("visualAnchors") or pool.get("identityAnchors") or [],
                            "reference_asset_ids": pool.get("referenceAssetIds") or []}
            return {"entity_id": cid, "name": None, "missing": True}

        env_ref = None
        env_id = shot.environment_id or shot.environmentId
        if env_id:
            env_ref = {"entity_id": env_id, "name": None}
            for e in vb.get("environments") or []:
                if e.get("environmentId") == env_id or e.get("id") == env_id:
                    env_ref = {"entity_id": env_id, "name": e.get("locationName") or e.get("name"),
                               "visual_anchors": e.get("visualAnchors") or [],
                               "biome": e.get("biome")}
                    break

        prop_refs = []
        _prop_pool = list(vb.get("props") or []) + list(vb.get("objects") or [])
        for pid in (shot.prop_ids or shot.propIds or []):
            pref: dict = {"entity_id": pid, "name": None}
            for pool in _prop_pool:
                if pool.get("propId") == pid or pool.get("objectId") == pid or pool.get("id") == pid:
                    pref = {"entity_id": pid, "name": pool.get("name"),
                            "description": pool.get("description")}
                    break
            prop_refs.append(pref)
        ...
        try:
            ...
            if vp_path.is_file():
                vp_data = json.loads(vp_path.read_text(encoding="utf-8"))
                vp_entry = next((e for e in vp_data.get("entries", [])
                                 if e.get("sceneId") == shot.parent_scene_id), None)
        except Exception:
            pass
        route = VisualRouter.route(shot, vp_entry)
        style = vb.get("projectStyle") or vb.get("visualStyle") or {}

        return {
            "shot_id": shot.shot_id,
            "scene_id": shot.parent_scene_id,
            "route": route.get("route"),
            "flow_handoff": {
                "image_prompt": shot.image_prompt or "",
                "negative_prompt": shot.negative_prompt or "",
                "character_references": [_char_ref(c) for c in (shot.subject_ids or shot.subjectIds or [])],
                "environment_reference": env_ref,
                "prop_references": prop_refs,
                "style": style,
            ...
            "veo_handoff": {
                ...
                "motion_blueprint": {
                    ...
                    "environment_motion": shot.environmental_action,
                    "lighting_atmosphere": shot.lighting_atmosphere,
                    "continuity_anchor": shot.continuity_anchor,
                    "duration_sec": shot.duration,
                    "constraints": shot.constraints,
                },
                "veo_motion_prompt": shot.veo_prompt or "",
                "required_references": {
                    "characters": [_char_ref(c) for c in (shot.subject_ids or shot.subjectIds or [])],
                    "environment": env_ref,
                    "props": prop_refs,
                },
            },
            "manual_only": True,
            "note": "Manual handoff — copy into Google Flow/Veo by hand. Nothing is uploaded automatically.",
        }
    except (FileNotFoundError, KeyError) as fe:
        raise HTTPException(status_code=404, detail=str(fe))
    except Exception as e:
        logger.exception(f"Failed to build handoff for {shot_id}: {e}")
        raise HTTPException(status_code=500, detail=str(e))
