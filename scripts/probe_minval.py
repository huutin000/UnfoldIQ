"""Probe: computed min-values on coarse violators (proper app load)."""
import json
import subprocess
import sys
import time
...
def main():
    ...
    if socket.socket().connect_ex(("127.0.0.1", 7860)) != 0:
        ...
        for _ in range(40):
            ...
            if socket.socket().connect_ex(("127.0.0.1", 7860)) == 0:
                break
        started = True
    Path("temp/probe_min_profile").mkdir(parents=True, exist_ok=True)
    chrome = subprocess.Popen(
        [r"C:\Program Files\Google\Chrome\Application\chrome.exe",
         f"--remote-debugging-port={PORT}",
         "--user-data-dir=" + str(Path("temp/probe_min_profile").resolve()),
    ...
    try:
        ...
        print("sheets:", E("document.styleSheets.length"), flush=True)
        print("coarseRules:", E("(function(){ const hits = [];"
                                " for (const sh of document.styleSheets) {"
                                "  let rules; try { rules = sh.cssRules; } catch (e) { continue; }"
                                "  const walk = function(rs){ for (const r of rs) {"
                                "   if (r.conditionText && r.conditionText.indexOf('coarse') >= 0)"
                                "    hits.push('COARSE:' + [...r.cssRules].map(x=>x.selectorText).join('|'));"
                                "   if (r.cssRules) walk(r.cssRules); } };"
                                "  walk(rules); } return JSON.stringify(hits); })()"), flush=True)
        print("modeBtn:", E("(function(){ const b = document.querySelector('.mode-toggle-group .mode-btn');"
